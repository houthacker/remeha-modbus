...
class RemehaHeatpumpManagedSchedulesSwitch(RemehaModbusSwitch):
    ...
    If set to 'off', the affected climate zones are put a mode allowing external
    preset management. If schedules are synchronized with the `scheduler` component,
    ...
    async def _async_switch_dhw_climates_to_eco_mode(self) -> None:
        ...
        for climate in coordinator.get_climates(lambda c: c.is_domestic_hot_water()):
            ...
            _LOGGER.debug(
                "Setting preset_mode of %s to %s to prevent conflicts between heat pump and HA.",
                entity_id,
                PRESET_ECO,
            )
    ...
    async def async_turn_off(self, **kwargs):
        ...
        await self._async_switch_dhw_climates_to_eco_mode()
